...
mv backup_submit.sh_$(date +%F).tar.gz projetos_backup/caseA-pre-process_$(date +%F).tar.gz
tar -cvf build.sbt_$(date +%F).tar.gz build.sbt
mv build.sbt_$(date +%F).tar.gz projetos_backup/caseA-pre-process_$(date +%F).tar.gz
tar -cvf project_$(date +%F).tar.gz project
mv project_$(date +%F).tar.gz projetos_backup/caseA-pre-process_$(date +%F).tar.gz
tar -cvf spark-warehouse_$(date +%F).tar.gz spark-warehouse
mv spark-warehouse_$(date +%F).tar.gz projetos_backup/caseA-pre-process_$(date +%F).tar.gz
